...
class Cell:
    ...
    def draw_move(self, to_cell, color=None):
        move = Line(
...
class Maze:
    ...
    def solve(self, trail_color):
        return self._solve_r(0, 0, trail_color)
    
    def _solve_r(self, i, j, trail_color):
        self._animate()
        self._cells[i][j].visited = True
        ...
        if i < self.num_cols - 1 and self._cells[i+1][j].visited == False and self._cells[i][j].has_right_wall == False:
            ...
            if self._solve_r(i+1, j, trail_color) == True:
                return True
        ...
        if j < self.num_rows - 1 and self._cells[i][j+1].visited == False and self._cells[i][j].has_bottom_wall == False:
            ...
            if self._solve_r(i, j+1, trail_color) == True:
                return True
...
class GUI:
    def __init__(self, width, height):
        ...
        tk.Button(text="Solve", command=self.solve).pack(side=tk.RIGHT, padx=10, pady=10)
        tk.Button(text="Generate", command=self.generate).pack(side=tk.RIGHT, padx=10, pady=10)
    ...
    def solve(self):
        if not self.active and self.maze != None:
            ...
            try:
                ...
                self.maze.erase_pos()
                self.maze.solve(self.trail_color)
                self.maze = None
                self.active = False
            except:
                self.active = False
    ...
    def generate(self):
        if not self.active:
            ...
            try:
                ...
                self.maze = Maze(20, 20, num_rows, num_cols, (self.width-40)/num_cols, (self.height-40)/num_rows, self.win, speed)
                self.active = False
            except:
                self.active = False
